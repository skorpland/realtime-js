...
export default class RealtimeClient {
   ...
   * @param options.workerUrl The URL of the worker script. Defaults to https://realtime.powerbase.club/worker.js that includes a heartbeat event call to keep the connection alive.
   */
  ...
  _resolveFetch = (customFetch?: Fetch): Fetch => {
    ...
    if (customFetch) {
      ...
      _fetch = (...args) =>
        import('@skorpland/node-fetch' as any).then(({ default: fetch }) =>
          fetch(...args)
        )
    } else {
      _fetch = fetch
